...
class AMT_RAPT_random(object):
    ...
    def execute(self, seed):
        ...
        for pure_cycle in range(1, 10000):
            ...
            if num_test_case_counter == 1:
                ...
                if isKilledMutant:
                    partition_index = ex_source_partition
                else:
                    partition_index = self.__rapt.get_next_partition_index()
                    ex_source_partition = partition_index
                    ex_follow_partition = partition_index
            ...
            test_case_index = int(selected_partition_test_cases[random.randint(
                0, len(selected_partition_test_cases) - 1)])
            ...
            ex_follow_partition = self.__exec_utl.get_follow_test_case_partition(
                test_case_index, ex_source_partition, selected_MR)
            ...
            for mutant_name in mutant_names_list:
                ...
                if isKilledMutant:
                    ...
                    # detect a fault, so we reward the source and follow partition
                    self.__rapt.add_reward(
                        ex_source_partition, ex_follow_partition)

                    # detect a fault, so we clear the punishement
                    self.__rapt.clear_punishement(
                        ex_source_partition, ex_follow_partition)
    ...
    def write_results_time_info(self):
        ...
        writer.write_info('AMT_RAPT_random.xlsx', self.get_all_average_list(), self.get_all_variance_list(),
