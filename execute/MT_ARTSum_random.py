...
from execute.ARTSum import ART
...
class MT_ART_RT(object):
    """
    ARTSum选择原始测试用例，随机的方法选择蜕变关系
    """
    ...
    def __init__(self):
        ...
        """
        self.tool = Utl()
        self.test_cases = self.tool.get_concrete_test_case()
        self.test_case_2_MRs = self.tool.get_test_case_2_MRs()
        self.art = ART()
    ...
    def execute(self, seed):
        ...
        for pure_cycle in range(1, 10000):
            ...
            if num_test_case_counter == 1:
                test_case_index = np.random.randint(1, 101194)
                self.art.init_s(self.tool.get_test_frame(test_case_index))
            else:
                self.art.update_candidate_test_case()
                test_case_index = self.art.get_source_test_case()
            ...
            # 获取衍生测试用例
            follow_pattern = self.exec_utl.generate_follow_test_case(
            ...
            for mutant_name in mutant_names_list:
                ...
                if selected_MR != 'MR11':
                    isKilledMutant = self.exec_utl.verify_result_not_MR11(selected_MR, str(seed),
                                                                          str(num_test_case_counter), mutant_name)
                else:
                    isKilledMutant = self.exec_utl.verify_result_MR11(str(seed), str(num_test_case_counter),
    ...
    def write_results_time_info(self):
        ...
        writer.write_info('test_results/MT_ART_RT.xlsx', self.get_all_average_list(), self.get_all_variance_list(),
